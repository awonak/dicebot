...
func (cmd *RollDiceCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	resp, err := c.RollDice(ctx, path, &payload, cmd.ContentType)
	if err != nil {
...
func (cmd *RollDiceCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
